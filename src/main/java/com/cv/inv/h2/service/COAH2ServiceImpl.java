/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.cv.inv.h2.service;

import com.cv.accountswing.service.*;
import com.cv.accountswing.dao.COAOpeningDao;
import com.cv.accountswing.entity.COALevel;
import com.cv.accountswing.entity.SystemProperty;
import com.cv.accountswing.entity.SystemPropertyKey;
import com.cv.inv.h2.dao.COAH2Dao;
import com.cv.inv.h2.entity.ChartOfAccountH2;
import java.util.List;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

/**
 *
 * @author winswe
 */
@Service
@Transactional
public class COAH2ServiceImpl implements COAH2Service {

    @Autowired
    private COAH2Dao dao;
    @Autowired
    private COAOpeningDao coaOpDao;
    @Autowired
    SeqTableService seqService;
    @Autowired
    SystemPropertyService spService;

    @Override
    public ChartOfAccountH2 save(ChartOfAccountH2 coa) {
        if (coa.getCode() == null || coa.getCode().isEmpty()) {
            coa.setCode(getCOACode(coa.getCompCode().toString()));
        }
        coa = dao.save(coa);
        return coa;
    }

    @Override
    public ChartOfAccountH2 save(ChartOfAccountH2 coa, String opDate) throws Exception {
        coaOpDao.insertCoaOpening(opDate, coa.getCompCode().toString(), "-", "-", coa.getCreatedBy());
        return save(coa);
    }

    @Override
    public ChartOfAccountH2 findById(String id) {
        ChartOfAccountH2 coa = dao.findById(id);
        return coa;
    }

    @Override
    public List<ChartOfAccountH2> search(String code, String name, String compCode,
            String level, String parent, String userParent, String usrCoaCode) {
        List<ChartOfAccountH2> listCOA = dao.search(code, name, compCode, level,
                parent, userParent, usrCoaCode);
        return listCOA;
    }

    @Override
    public int delete(String code, String compCode) {
        int cnt = dao.delete(code, compCode);
        return cnt;
    }

    @Override
    public List<ChartOfAccountH2> getParent(String compCode) {
        List<ChartOfAccountH2> listCOA = dao.getParent(compCode);
        return listCOA;
    }

    @Override
    public List<COALevel> getParentChildCOA(String compCode) {
        List<COALevel> listCOAL = dao.getParentChildCOA(compCode);
        return listCOAL;
    }

    @Override
    public List<ChartOfAccountH2> getCOALevel3Above(String compCode) {
        List<ChartOfAccountH2> listCOA = dao.getCOALevel3Above(compCode);
        return listCOA;
    }

    @Override
    public List<ChartOfAccountH2> getCOALevel2Above(String compCode) {
        List<ChartOfAccountH2> listCOA = dao.getCOALevel2Above(compCode);
        return listCOA;
    }

    @Override
    public List<ChartOfAccountH2> getAllChild(String parent, String compCode) {
        List<ChartOfAccountH2> listCOA = dao.getAllChild(parent, compCode);
        return listCOA;
    }

    @Override
    public List<ChartOfAccountH2> getCompanyCOA(String compCode) {
        List<ChartOfAccountH2> listCOA = dao.getCompanyCOA(compCode);
        return listCOA;
    }

    @Override
    public List<ChartOfAccountH2> getCompanyCOA(String compCode, String deptId, String projectId) {
        List<ChartOfAccountH2> listCOA = dao.getCompanyCOA(compCode, deptId, projectId);
        return listCOA;
    }

    @Override
    public List<ChartOfAccountH2> getChild(String compCode, String parent) {
        List<ChartOfAccountH2> listCOA = dao.getChild(compCode, parent);
        return listCOA;
    }

    private String getCOACode(String compCode) {
        SystemPropertyKey spk = new SystemPropertyKey("system.coa.code.length",
                Integer.parseInt(compCode));
        SystemProperty sp = spService.findById(spk);
        int ttlLength = Integer.parseInt(sp.getPropValue());
        int seqNo = seqService.getSequence("COA", "-", compCode);
        String coaCode = compCode + "-" + String.format("%0" + ttlLength + "d", seqNo);
        return coaCode;
    }

    @Override
    public List<ChartOfAccountH2> searchWhereIn(String strList, String compCode) {
        return dao.searchWhereIn(strList, compCode);
    }
}
