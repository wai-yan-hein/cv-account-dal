...
import com.cv.account.api.util.Global;
...
public class COAOpeningDaoDImpl extends AbstractDao<Long, AccOpeningD> implements COAOpeningDaoD {
    ...
    public void genTriBalance1(String compCode, String fromDate, String opDate, String tranDate, String coaCode, String currency, String dept, String cvId, String userId) throws Exception {
        ...
        String strSqlFilter = "insert into tmp_op_filter(comp_code, coa_code, dept_id, curr_id, tran_source, op_date, user_id,machine_id)"
                ...
                + "'" + userId + "','" + Global.machineId.toString() + "'  from v_coa_curr vcc left join (\n"
        ...
        String strSql = "insert into tmp_op_cl(coa_code, curr_id, user_id, dr_amt, cr_amt,machine_id) \n"
                + "select coa_code, curr_id, '" + userId + "', if(sum(dr_amt-cr_amt)>0, sum(dr_amt-cr_amt),0), if(sum(dr_amt-cr_amt)<0, sum(dr_amt-cr_amt)*-1,0), '" + Global.machineId.toString() + "'\n"
                + "from (\n"
    ...
    public void genOpBalanceGL1(String coaCode, String opDate, String clDate, int level, String curr, String userId, String dept) throws Exception {
        ...
        String insertSql = "insert into tmp_op_filter(comp_code, coa_code, op_tran_id_d, curr_id, op_date, user_id,machine_id) \n"
                + "select coa.comp_code,coa.coa_code,op.coa_op_id,ifnull(op.cur_code,'" + curr + "'),op.op_date,'" + userId + "'," + Global.machineId + "\n"
                + "from chart_of_account coa left join coa_opening op \n"
                + "on coa.coa_code = op.source_acc_id and coa.comp_code = op.comp_id\n"
        ...
        String opSql = "insert into tmp_op_cl(coa_code, curr_id, user_id, opening, dr_amt, cr_amt,machine_id) \n"
                + "select coa_code, curr_id, '" + userId + "', sum(balance), 0, 0," + Global.machineId + "\n"
                + "from (\n"
                ...
                + "	where  tof.comp_code = coa.comp_id and tof.curr_id = coa.cur_code \n"
                ...
                + "     from tmp_op_filter tof, gl\n"
                + "	where tof.comp_code = gl.comp_id and (tof.coa_code = gl.source_ac_id or \n"
    ...
    public void getOpBalanceByTrader(String coaCode, String opDate, String clDate, int level, String curr, String userId, String dept, String cvId) throws Exception {
        ...
        String insertSql = "insert into tmp_op_filter(comp_code, coa_code, op_tran_id_d, curr_id, op_date, user_id,cv_id,machine_id) \n"
                + "select coa.comp_code,coa.coa_code,op.coa_op_id,ifnull(op.cur_code,'" + curr + "'),op.op_date,'" + userId + "',op.cv_id," + Global.machineId + "\n"
                + "from chart_of_account coa left join coa_opening op \n"
        ...
        String strSql = "insert into tmp_op_cl(coa_code, curr_id, user_id, opening, dr_amt, cr_amt,machine_id) \n"
                ...
                + "ifnull(gl.dr_amt,0) dr_amt, ifnull(gl.cr_amt,0),tof.cv_id \n"
                + "from tmp_op_filter tof, gl\n"
                + "where tof.comp_code = gl.comp_id and tof.cv_id = gl.cv_id\n"
